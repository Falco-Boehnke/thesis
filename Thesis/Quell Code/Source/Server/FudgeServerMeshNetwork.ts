...
export class FudgeServerMeshNetwork implements FudgeNetwork.MeshNetworkSignalingServer {
    ...
    public sendClientListToClient(): void {
        if (this.peerMeshReadyClientCollection.length == 0) {
            console.log(this.peerMeshReadyClientCollection.length);
            console.log("All Clients connected");
        } else {
            let clientToSendTo: FudgeNetwork.ClientDataType = this.peerMeshReadyClientCollection[0];
    ...
    public setClientReadyFlag = (_clientId: string) => {
        let clientReady: FudgeNetwork.ClientDataType = this.searchUserByUserIdAndReturnUser(_clientId, this.connectedClientsCollection);
