...
export class ClientManagerFullMeshStructure implements FudgeNetwork.ClientManagerMeshTemplate {
    ...
    establishedDataChannelsWithRemoteIds!: [string, RTCDataChannel];
    ...
    public beginnMeshConnection(_meshClientMessage?: FudgeNetwork.NetworkMessageServerSendMeshClientArray): void {
    ...
    public sendReadySignalToServer(): void {
        let readyToConnectMessage: FudgeNetwork.NetworkMessageClientReady = new FudgeNetwork.NetworkMessageClientReady(this.localClientID);
        this.sendMessageToSignalingServer(readyToConnectMessage);
    }

    public sendFinishingSignal(): void {
        let connectedToMeshMessage: FudgeNetwork.NetworkMessageClientIsMeshConnected = new FudgeNetwork.NetworkMessageClientIsMeshConnected(this.localClientID);
    ...
    public createMeshClientAndAddPeerConnection(): FudgeNetwork.ClientDataType {
        let newMeshClient: FudgeNetwork.ClientDataType = new FudgeNetwork.ClientDataType();
    ...
    public addNewMeshClientToMeshClientCollection(_meshClient: FudgeNetwork.ClientDataType): void {
    ...
    public beginPeerConnectionNegotiation = (_userIdToOffer: string): void => {
        ...
        let currentlyNegotiatingPeer: FudgeNetwork.ClientDataType = this.createMeshClientAndAddPeerConnection();
    ...
    public addReceivedCandidateToPeerConnection = async (_receivedIceMessage: FudgeNetwork.NetworkMessageIceCandidate) => {
        let relevantClient: FudgeNetwork.ClientDataType | null = this.searchNegotiatingClientById(_receivedIceMessage.originatorId, this.remoteMeshClients);
    ...
    public parseReceivedMessageAndReturnObject = (_receivedMessage: MessageEvent): FudgeNetwork.NetworkMessageMessageBase => {
        // tslint:disable-next-line: no-any
    ...
    private searchNegotiatingClientById(_idToSearch: string, _arrayToSearch: FudgeNetwork.ClientDataType[]): FudgeNetwork.ClientDataType | null {
        _arrayToSearch.forEach(meshClient => {
